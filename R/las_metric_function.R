las_cld_metrics <- function(z) {
  ...
  z_max = max(z_gt1, na.rm = TRUE)
  z_mean = mean(z_gt1, na.rm = TRUE)
  ...
  lf_12 = sum(z >= 1 & z < 2) / sum(z < 2)
  lf_23 = sum(z >= 2 & z < 3) / sum(z < 3)
  lf_34 = sum(z >= 3 & z < 4) / sum(z < 4)
  lf_45 = sum(z >= 3 & z < 4) / sum(z < 5)
  lf_56 = sum(z >= 5 & z < 6) / sum(z < 6)
  lf_67 = sum(z >= 6 & z < 7) / sum(z < 7)
  lf_78 = sum(z >= 7 & z < 8) / sum(z < 8)
